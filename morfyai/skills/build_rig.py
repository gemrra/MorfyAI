...
def _verify_rig(result, rig_type):
    ...
    path = (result.get("skeleton_output") or result.get("posed_output")
            or result.get("deform_node") or result.get("display_node"))
...
SKILL_INFO = {
    ...
    "description": (
        ...
        "Typical order: skeleton -> skinning -> pose. After each step a data check runs "
        "automatically; follow it with a VIEWPORT capture (or the visual_check skill) — rig "
        "correctness is mostly visual (joints inside the mesh? smooth deformation?)."
    ),
    "parameters": {
        "rig_type": {
            "type": "string",
            "description": "Which rigging task to perform.",
            "enum": ["skeleton", "skinning", "pose"],
            "required": True,
        },
        # skeleton
...
def run(rig_type=None, container_name=None, preset=None, height=None,
        ...
        use_ik=None, test_joint=None, test_rotate=None, verify=True):
    import inspect
