...
_SIM_POLICY = (
    ...
    "- skill__build_sim AUTO-VERIFIES the result and returns a 'verification' block (cooks the sim, reports "
    "points/prims/errors). If verification.ok is false OR 'needs_fix' is set, you MUST diagnose and fix it "
    "(inspect the reported errors, check the solver inputs/wiring, adjust the offending node) and re-verify — "
    "do NOT tell the user the sim is done while verification.ok is false. If verification.ok is true, briefly "
    "confirm the counts look right for what was asked.\n"
